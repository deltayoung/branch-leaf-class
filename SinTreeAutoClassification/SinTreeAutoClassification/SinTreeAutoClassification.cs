...
namespace SinTreeAutoClassification
{
  ...
  public class SinTreeAutoClassification : IDisposable
  {
    ...
    public void Classify(string SingleTreeLasFilePath)
    {
      ...
      var xmlFilePath = SingleTreeLasFilePath.Replace("las", "xml");
      ...
      for (int ix = 0; ix < lasPoints.Count; ix++)
      {
        ...
        lasPointDataArray[ix].isTrunk = IsTrunkPoint(lasPoints[ix], treeLoc); // a small subsection of the points is then designated as part of a trunk.. 
      }
          ...
          if (reader.IsStartElement())
          {
            ...
            switch (reader.Name)
            {
              case "CORD":
                if (reader.Read())
                {
                  var value = reader.Value.Trim().Split(' ');
                  if (value.Length == 3)
                  {
